...
>对于叭啵棋而言，比起纯卷积/卷积-注意力混合架构，收敛速度大幅加快，且更少的参数量就可以达到数据上限  
>可能的原因是卷积层特征提取能力远不如注意力层，却延长了梯度流通路径  
>同时，卷积层不具有全局视野，其长程视野较为间接，需要通过卷积核互相交换，而模型训练不可能达到绝对精确，因此对长程交互感知模糊，存在误差  
>粗略测试大约1/3的参数量可以达到卷积残差块的效果  
>260106加入PoPE和mHC，使用Muon优化器  
>260323加入attention residuals
```
