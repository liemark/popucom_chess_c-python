...
// ... (Node, NodeStore, MCTSSearch, MCTSManager 结构和之前一样，此处省略以保持简洁) ...
// The Node, NodeStore, MCTSSearch, and MCTSManager implementation remains the same as the previous optimized version.
// For brevity, only the new C API function is shown in detail below, along with the other existing API functions.
