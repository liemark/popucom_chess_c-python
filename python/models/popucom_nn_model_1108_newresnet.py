...
try:
    ...
    print("注意: 无法导入 popucom_nn_interface.py。使用默认值继续。")
...
class NestedBottleneckBlock(nn.Module):
    """
    -- 新增模块 --
    根据 KataGo 的文档实现的嵌套瓶颈残差块。
    这个结构在计算上更高效，它在瓶颈内使用了两个标准的残差块。
    """

    def __init__(self, num_filters):
        super(NestedBottleneckBlock, self).__init__()
        bottleneck_filters = num_filters // 2

        # 同样采用前激活 (Pre-activation) 风格

        # 1. 1x1 降维卷积 (256 -> 128)
        self.bn1 = nn.BatchNorm2d(num_filters)
        self.relu1 = nn.ReLU(inplace=True)
        self.conv_1x1_reduce = nn.Conv2d(num_filters, bottleneck_filters, kernel_size=1, bias=False)

        # 2. 两个嵌套的内部残差块 (在 128 维度上)
        self.inner_res_block_1 = ResidualBlock(bottleneck_filters)
        self.inner_res_block_2 = ResidualBlock(bottleneck_filters)

        # 3. 1x1 升维卷积 (128 -> 256)
        # 注意：这里的 BN 和 ReLU 是在 inner_res_block_2 的输出上，
        # 遵循 KataGo 图示 (bottlenecknestedresblock.png) 的前激活逻辑
        self.bn2 = nn.BatchNorm2d(bottleneck_filters)
        self.relu2 = nn.ReLU(inplace=True)
        self.conv_1x1_restore = nn.Conv2d(bottleneck_filters, num_filters, kernel_size=1, bias=False)
    ...
    def forward(self, x):
        ...
        # 1x1 降维
        out = self.relu1(self.bn1(x))
        out = self.conv_1x1_reduce(out)

        # 内部残差块
        out = self.inner_res_block_1(out)
        out = self.inner_res_block_2(out)

        # 1x1 升维
        out = self.relu2(self.bn2(out))
        out = self.conv_1x1_restore(out)
...
class PomPomNN(nn.Module):
    """
    泡姆棋的神经网络模型。
    MODIFIED: 主干网络现在使用 NestedBottleneckBlock。
    """
    ACTUAL_INPUT_CHANNELS = NUM_INPUT_CHANNELS + 2

    def __init__(self, num_res_blocks=14, num_filters=256):
        ...
        # --- 优化点 ---
        # 将主干的残差块替换为嵌套瓶颈块
        # KataGo 'b18c384nbt' 中的 'b18' 指的就是 18 个这样的嵌套块
        self.res_blocks = nn.ModuleList([NestedBottleneckBlock(num_filters) for _ in range(num_res_blocks)])
        # -----------
...
if __name__ == "__main__":
    ...
    # 使用优化后的模型
    model = PomPomNN(num_res_blocks=10, num_filters=128)  # 示例：使用 10 个嵌套块和 128 个过滤器
    model.eval()

    with torch.no_grad():
        policy_logits, value, soft_policy_logits = model(dummy_input)

    print("--- 优化后的 PomPomNN 模型 ---")
