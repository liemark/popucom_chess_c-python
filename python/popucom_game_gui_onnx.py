...
# popucom_nn_model 不再需要，因为我们直接使用onnx
...
# --- 全局常量 ---
MODEL_PATH, BLACK_PLAYER, WHITE_PLAYER = "model.onnx", 0, 1
...
class PomPomGUI:
    def __init__(self, master):
        ...
        self.ai_model_session = self._load_ai_model()
    ...
    def _load_ai_model(self):
        try:
            ...
            messagebox.showerror("模型加载失败", f"无法加载ONNX模型: {e}\n请确保 'model.onnx' 文件存在，并检查环境依赖。")
            return None
    ...
    def _stop_ai_thread(self):
        if self.ai_thread and self.ai_thread.is_alive():
            self.game_running = False
            self.ai_thread.join(timeout=0.2)
    ...
    def draw_board(self):
        ...
        board_pixel_size = self.cell_size * BOARD_SIZE
        ...
        for r in range(BOARD_SIZE):
            for c in range(BOARD_SIZE):
                ...
                self.canvas.create_rectangle(x1, y1, x2, y2, fill=color, outline=GRID_LINE_COLOR)
        for r in range(BOARD_SIZE):
            for c in range(BOARD_SIZE):
                sq = r * BOARD_SIZE + c
                center_x, center_y, radius = c * self.cell_size + self.cell_size / 2, r * self.cell_size + self.cell_size / 2, self.cell_size / 2.3
                p_color = None
        ...
        if self.analysis_data:
            fnt = font.Font(family='Helvetica', size=max(5, int(self.cell_size / 14)), weight='bold')
            for (r, c), data in self.analysis_data.items():
                ...
                self.canvas.create_text(center_x, center_y - self.cell_size * 0.25, text=f"胜率: {win_rate:.1f}%",
                                        fill="blue" if data['q'] > 0 else "purple", font=fnt)
                self.canvas.create_text(center_x, center_y, text=f"访问: {data['visits']}", fill="black", font=fnt)
                self.canvas.create_text(center_x, center_y + self.cell_size * 0.25, text=f"PUCT: {data['puct']:.3f}",
                                        fill="#006400", font=fnt)
        if self.best_puct_move:
            ...
            self.canvas.create_rectangle(c * self.cell_size + 3, r * self.cell_size + 3, (c + 1) * self.cell_size - 3,
                                         (r + 1) * self.cell_size - 3, outline="orange", width=3, dash=(4, 4))
    ...
    def _on_listbox_select(self, event):
        ...
        if 0 <= selected_index < len(self.listbox_nodes):
            ...
            if node_to_visit != self.current_node: self._navigate_to_node(node_to_visit)
    ...
    def _run_mcts_loop(self, num_sims, is_analysis=False):
        if not self.ai_model_session:  # MODIFIED: Check for onnx session
            print("错误: AI模型会话未初始化，无法进行MCTS。")
            if is_analysis:
        ...
        while c_lib.mcts_get_simulations_done(self.mcts_manager_gui, 0) < num_sims:
            ...
            if c_lib.mcts_run_simulations_and_get_requests(self.mcts_manager_gui, boards, indices, 1) > 0:
                ...
                ort_inputs = {self.ai_model_session.get_inputs()[0].name: input_tensor_np}
                ort_outs = self.ai_model_session.run(None, ort_inputs)

                policy_logits = ort_outs[0]
                value_output = ort_outs[1]
