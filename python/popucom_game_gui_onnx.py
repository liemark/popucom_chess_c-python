...
# 确保从正确的文件导入
# from popucom_nn_model import PomPomNN # 使用ONNX模型不再需要
...
# --- 全局常量 ---
MODEL_PATH, BLACK_PLAYER, WHITE_PLAYER = "model.onnx", 0, 1  # 改为onnx
...
class PomPomGUI:
    def __init__(self, master):
        ...
        # --- FIX: 将加载的会话保存到正确的属性 self.ai_model_session ---
    ...
    def _load_ai_model(self):
        ...
        try:
            ...
            print(f"可用的ONNX Runtime Provider: {providers}")

            provider_to_use = 'CPUExecutionProvider' # 使用cpu版本已经足够，避免兼容性问题
            #if 'CUDAExecutionProvider' in providers:
            #    provider_to_use = 'CUDAExecutionProvider'


            print(f"将使用: {provider_to_use}")
