...
class MultiHeadSelfAttentionWithRelativeBias(nn.Module):
    ...
    def forward(self, x):
        ...
        relative_x_bias = self.relative_x_bias_embedding[:, relative_x_indices]  # (num_heads, seq_len, seq_len)
        relative_y_bias = self.relative_y_bias_embedding[:, relative_y_indices]  # (num_heads, seq_len, seq_len)
...
class PomPomNN(nn.Module):
    ...
    def __init__(self, num_res_blocks=5, num_filters=96, num_attention_heads=4):
        """
        ...
        # Input: Main network output (batch_size, num_filters, BOARD_SIZE, BOARD_SIZE)
        # Output: Probability distribution over possible moves on a 9x9 board (after softmax normalization)
        self.soft_policy_conv = nn.Conv2d(num_filters, 2, kernel_size=1,
                                     bias=False)  # 1x1 convolution, outputs 2 feature maps
        self.soft_policy_bn = nn.BatchNorm2d(2)
        self.soft_policy_relu = nn.ReLU(inplace=True)
        # Add Global Pooling Bias after the policy head's convolution output
        self.soft_global_pool_bias_policy = GlobalPoolingBias(2)  # Policy head output channel is 2
    ...
    def forward(self, x):
        ...
        policy = policy.reshape(policy.size(0), -1)  # Flatten (batch_size, 2 * 9 * 9)
        ...
        value = value.reshape(value.size(0), -1)  # Flatten (batch_size, 9 * 9)
        ...
        soft_policy = soft_policy.reshape(soft_policy.size(0), -1)  # Flatten (batch_size, 2 * 9 * 9)
...
if __name__ == "__main__":
    ...
    model = PomPomNN(num_res_blocks=5, num_filters=96, num_attention_heads=4)
    ...
    print(f"Value output range: [-1, 1] (achieved via Tanh activation)")

    print(f"\nOwnership output shape: {ownership.shape}")
    print(f"Ownership output example (top-left portion):\n{ownership[0, :3, :3]}")
    print(f"Ownership output range: [-1, 1] (achieved via Tanh activation)")
