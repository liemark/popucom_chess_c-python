...
def augment_data(state, policy):
    """
    对棋盘状态和策略进行随机的旋转和翻转。
    MODIFIED: Removed legal_moves augmentation.
    """
...
class PopucomDataset(Dataset):
    ...
    MODIFIED: Handles the new data format (state, policy, value).
    """
    ...
    def __getitem__(self, idx):
        # MODIFIED: Data format is now (state, policy, value)
        state, policy, value = self.data[idx]
        if self.augment:
            state, policy = augment_data(state, policy)
        # MODIFIED: Return the simplified tuple
...
def get_args():
    """
    解析命令行参数
    MODIFIED: Removed --legal-moves-weight argument.
    """
...
def train_model(args):
    ...
    # MODIFIED: Removed legal moves loss function

    scaler = torch.amp.GradScaler(device=device.type, enabled=(device.type == 'cuda'))

    start_time = time.time()
    for epoch in range(args.epochs):
        # MODIFIED: Removed 'legal_moves' from losses dict
        losses = {'total': 0.0, 'policy': 0.0, 'value': 0.0, 'soft_policy': 0.0}

        # MODIFIED: Dataloader now yields 3 items
        for states, target_policies, target_values in dataloader:
            ...
            with torch.amp.autocast(device_type=device.type, enabled=(device.type == 'cuda')):
                # MODIFIED: Model returns 3 outputs
                ...
                # MODIFIED: Removed legal_moves loss from total loss
